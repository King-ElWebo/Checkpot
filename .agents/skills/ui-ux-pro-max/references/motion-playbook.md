...
This is a long-form reference for advanced motion, interaction, implementation patterns, and debugging. It is not an activation prompt and should not be loaded for every UI task.

The binding order is: customer-specific decisions in `docs/DESIGN-SYSTEM.md`, then `motion-principles.md`, then this playbook. Examples and statements framed as “always” or “never” here are heuristics to test in context, not universal requirements. Accessibility and reduced-motion requirements remain mandatory.

Build interfaces where every detail compounds into something that feels right. In a world where much software is merely good enough, craft and taste are differentiators.
...
## Optional Review Format

When several comparable issues are present, a Before/After table can make the review easier to scan:
...
Less scannable for a multi-item comparison:
...
A compact comparison format uses | Before | After | Why | columns, one row per issue. For one simple finding, plain prose may be clearer.
...
| Frequency                                                   | Decision                     |
| ----------------------------------------------------------- | ---------------------------- |
| 100+ times/day (keyboard shortcuts, command palette toggle) | Immediate feedback; omit decorative delay |
| Tens of times/day (hover effects, list navigation)          | Remove or drastically reduce |
| Occasional (modals, drawers, toasts)                        | Standard animation           |
| Rare/first-time (onboarding, feedback forms, celebrations)  | Can add delight              |

Keyboard-initiated actions should provide immediate feedback. Subtle state continuity can remain when it does not delay the action, but repeated expert workflows should avoid ornamental motion.
